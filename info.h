...
GROUP BY \"composition\".\"id\" " };
    ...
    const char* insert[6] = {
        "insert into client(name, login, password) values(\"Nevar Pavel\", \"npavel\", \"1234567\");",
        "insert into composition(title, cost) values(\"Winter\", \"290\");",
        "insert into compositionInOrder(order_id, composition_id, count) values(\"5\", \"5\", \"3\");",
        "insert into flower(name, sort, cost) values(\"daffodil\", \"yellow\", \"90\");",
        "insert into flowerInComposition(flower_id, composition_id, count) values(\"6\", \"2\", \"3\");",
        "insert into \"order\"(client_id, accept_date, urgency_id) values(\"5\", \"20210212\", \"2\");"
    };

    const char* delete_query[6] =
        {"delete from client where name = \"Nevar Pavel\";", "delete from composition where title like \"W%\";",
        "delete from compositionInOrder where order_id = \"5\";", "delete from flower where name like \"d%\";",
        "delete from flowerInComposition where flower_id = \"6\";", "delete from \"order\" where accept_date = \"20210212\";" };

    const char* update[6] =
    {
        "update client set login = \"Pashka\" where name = \"Nevar Pavel\";", "update composition set cost = \"320\" where title like \"W%\";",
        "update compositionInOrder set count = \"5\" where order_id = \"5\";", "update flower set sort = \"white\" where name like \"d%\";",
        "update flowerInComposition set count = \"7\" where flower_id = \"6\";", "update \"order\" set urgency_id = \"1\" where accept_date = \"20210212\";"
    };
